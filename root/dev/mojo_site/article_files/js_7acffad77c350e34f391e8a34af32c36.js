...
//code to redirect user to an interstital page between first and second page of a visit
//on first page the mj_inter cookie is set to 1
//on second page, if the mj_inter cookie is found set to 1 user is redirected to interstitial
//on the interstitial page the mj_inter cookie is set to 2
//there is also geotargeted ad code running to set the cookie to 3 for non us visitors
...
var mj_inter = Get_Cookie("mj_inter");
var mj_drupaluid = Get_Cookie("DRUPAL_UID");
var mj_interpage1 = "http://motherj";
var mj_interpage2 = "ones.com/transit";
var mj_interpage3 = "ion/inter.php?dest=";
var mj_interloc = window.location.href;
var mj_inter_browser=navigator.userAgent;


if ((mj_inter_browser.indexOf("iPad") == -1) && (mj_inter_browser.indexOf("Opera") == -1)){
  if (mj_drupaluid == "") {
    if (mj_inter == "") {
      var mj_interdate = new Date();
      var mj_interdatemili = mj_interdate.getTime() + (30 * 60 * 1000);
      mj_interdate.setTime(mj_interdatemili);
      document.cookie = "mj_inter=1;expires=" + mj_interdate.toGMTString() + ";path=/;domain=.motherjones.com";
    } else if (mj_inter == "1") {
      var mj_interrand = Math.floor(Math.random()*10);
      if (mj_interrand < 5) {
        var mj_internode = mj_interloc.indexOf("node");
        var mj_interadmin = mj_interloc.indexOf("admin");
        var mj_interuser = mj_interloc.indexOf("user");
        var mj_interthanks = mj_interloc.indexOf("thanks");
        var mj_intercomment = mj_interloc.indexOf("comment");
        var mj_interbw = mj_interloc.indexOf("bearingwitness");
        var mj_interabout = mj_interloc.indexOf("about");
        var mj_intersearch = mj_interloc.indexOf("search");
        if ((mj_interadmin == -1) && (mj_interuser == -1) && (mj_internode == -1) && (mj_interthanks == -1) && (mj_intercomment == -1) && (mj_interbw == -1) && (mj_interabout == -1) && (mj_intersearch == -1)) {
      
        window.location = mj_interpage1 + mj_interpage2 + mj_interpage3 + mj_interloc;
        }
      }
    }
  }
